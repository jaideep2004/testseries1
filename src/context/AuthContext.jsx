...
// 			// Get tokens from sessionStorage instead of sessionStorage
...
export const AuthProvider = ({ children }) => {
	...
	// Modified to use both localStorage and sessionStorage
	const storeToken = (token, isAdmin = false) => {
		const tokenKey = isAdmin ? "adminToken" : "userToken";
		try {
	...
	// Modified to check both storage types
	const getStoredToken = (isAdmin = false) => {
		const tokenKey = isAdmin ? "adminToken" : "userToken";
		try {
	...
	useEffect(() => {
		const checkStoredToken = async () => {
			// Get tokens from sessionStorage instead of sessionStorage
			const userToken = getStoredToken(false);
			const adminToken = getStoredToken(true);
	...
	const login = async (credentials, isAdminLogin = false) => {
		try {
			...
			if (data.token) {
				storeToken(data.token, isAdminLogin);
				api.defaults.headers.common["Authorization"] = `Bearer ${data.token}`;
	...
	const clearAuth = () => {
		["userToken", "adminToken"].forEach((tokenKey) => {
			localStorage.removeItem(tokenKey);
			sessionStorage.removeItem(tokenKey);
		});
		delete api.defaults.headers.common["Authorization"];
