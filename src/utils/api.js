...
// Using local API for development, uncomment the production URL for deployment
...
api.interceptors.response.use(
	...
	async (error) => {
		...
		if (error.config.responseType === "blob" && error.response?.data) {
			try {
				...
				error.response.data = errorData;
					...
					error.response.data = { message: text || "Download failed. Please try again." };
				}
			} catch (e) {
		...
		if (error.code === 'ECONNABORTED') {
			error.message = "Request timed out. The file might be too large or the server is busy. Please try again later.";
		}
...
export const downloadFile = async (url, filename, fileType = null) => {
	try {
		...
		if (url.includes('drive.google.com')) {
			...
			const finalFilename = baseFilename.replace(/[^a-zA-Z0-9._-]/g, '_') + fileExtension;
			...
			if (!downloadUrl.includes('export=download') && !downloadUrl.includes('export?format=')) {
		...
		const isCloudUrl = url.includes('storage.googleapis.com') || 
		...
		let contentType = response.headers['content-type'] || fileType || 'application/octet-stream';
 ...
 */
export const uploadToCloudStorage = async (file, contentType = 'application/pdf') => {
	try {
